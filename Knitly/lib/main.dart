            ...
            parts: p.parts.map((pt) {
              if (pt.id == partId) {
                return ProjectPart(id: pt.id, name: pt.name, counters: [
                  ...pt.counters,
                  KnitCounter(
                      id: const Uuid().v4(), name: name, colorValue: 0xFF81C784)
                ]);
              }
              return pt;
            }).toList());
              ...
              counters: pt.counters.map((c) {
                ...
                if (c.id == triggerCounterId) {
                  ...
                  if ((c.linkedCounterId == triggerCounterId &&
                    ...
                    shouldUpdate = true;
                  } else if (triggerCounter.linkedCounterId == c.id &&
                ...
                if (shouldUpdate) {
                  return KnitCounter(
                      id: c.id,
                      name: c.name,
                      currentRow: (c.currentRow + delta).clamp(0, 9999),
                      reminders: c.reminders,
                      linkedCounterId: c.linkedCounterId,
                      linkMode: c.linkMode,
                      colorValue: c.colorValue);
    ...
    return MaterialApp(
      ...
      theme: ThemeData(
        ...
        colorScheme: ColorScheme.fromSeed(
            seedColor: const Color(0xFFE1F5FE),
...
class _PartDetailScreenState extends ConsumerState<PartDetailScreen> {
        ...
        actions: [
          ...
          TextButton(
            onPressed: () {
              ...
              if (val != null) {
                final delta = val - counter.currentRow;
                ref.read(projectProvider.notifier).updateRow(
                    widget.projectId, widget.partId, counter.id, delta);
              }
              Navigator.pop(ctx);
            },
  ...
  Widget _buildSecondaryTracker(
      BuildContext context, KnitCounter counter, List<KnitCounter> all) {
    Color col = Color(counter.colorValue ?? 0xFF81C784);
    return Card(
      ...
      child: Padding(
        ...
        child: Column(
          ...
          children: [
                ...
                Expanded(
                    child: Text(counter.name,
                        style: TextStyle(
                            ...
                            color: col),
                        maxLines: 1)),
                _menu(context, counter, all, compact: true)
              ],
            ),

            // NEW: Added proportional circle layout matching the global design
            Row(
              mainAxisAlignment: MainAxisAlignment.spaceBetween,
              children: [
                _btn(
            ...
            Text(
                counter.linkMode == 'none'
                    ? "Independent"
                    : (counter.linkMode == 'sync'
                        ? "Identical Sync"
                        : "One-Way Stream"),
                style: TextStyle(
                    fontSize: 9,
  ...
  Widget _menu(BuildContext context, KnitCounter counter, List<KnitCounter> all,
    ...
    return SizedBox(
      ...
      child: PopupMenuButton<String>(
        padding: EdgeInsets.zero,
        color: Colors.white,
        icon: Icon(Icons.more_vert, size: compact ? 18 : 22),
        itemBuilder: (c) => [
          const PopupMenuItem(
              value: 'reminders', child: Text("Configure Reminders")),
          const PopupMenuItem(value: 'reset', child: Text("Reset to 0")),
          const PopupMenuItem(value: 'color', child: Text("Set Palette Color")),
          if (counter.id != all.first.id)
            const PopupMenuItem(value: 'link', child: Text("Link Settings")),
        ...
        onSelected: (v) {
          ...
          if (v == 'reset') {
            final delta = 0 - counter.currentRow;
            ref
                .read(projectProvider.notifier)
                .updateRow(widget.projectId, widget.partId, counter.id, delta);
          }
          if (v == 'rename') {
            _showRenameDialog(
                context,
  ...
  // NEW: Revamped intuitive option description strings for linking behavior logic
  void _showLinkDialog(
      BuildContext context, KnitCounter counter, List<KnitCounter> all) {
    String? selectedLink = counter.linkedCounterId;
    String selectedMode =
        counter.linkMode == 'none' ? 'follow' : counter.linkMode;
                      ...
                      DropdownButtonFormField<String?>(
                        ...
                        items: [
                          const DropdownMenuItem(
                              value: null,
                              child: Text("No Link (Standalone Counter)")),
                          DropdownMenuItem(
